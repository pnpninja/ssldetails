...
final class ServerHello {
		...
		throws IOException
	{
		...
		protoVersion = Decryption.dec16be(buf, 0);
		ptr += 2;
		...
		serverTime = 1000L * (Decryption.dec32be(buf, ptr) & 0xFFFFFFFFL);
		ptr += 32;
		...
		cipherSuite = Decryption.dec16be(buf, ptr);
	...
	private void processCertificate(byte[] buf)
	{
		...
		int len1 = Decryption.dec24be(buf, 0);
		if (len1 != buf.length - 3) {
			return;
		}
		int len2 = Decryption.dec24be(buf, 3);
		...
		try {
			...
			serverCertHash = SHA1Utils.doSHA1(ec);
